...
async def filter_by_status_menu(message: Message):
    """Показывает инлайн-клавиатуру для статусов"""
...
async def filter_orders_by_status_handler(callback: CallbackQuery):
    ...
    if status == 200 and orders:
        ...
        if filtered_orders:
            ...
            await callback.message.answer(f'Заказов со статусом {ORDER_STATUSES.get(status_filter, status_filter)} не найдено 📭', reply_markup=admin_filter_keyboard)
...
async def back_handler(message: Message):
    """Кнопка назад - возвращает админа в главное меню"""
    telegram_id = message.from_user.id
    role = user_roles.get(telegram_id)
    if role == 'admin':
